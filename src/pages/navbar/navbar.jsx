...
const Navbar = () => {
    return (
        <nav className="bg-white flex flex-col justify-between h-[791px] rounded-xl">
            <ul className="flex flex-col items-center">
                {
                    navbarInfo.map((item, i) => (<Link to={item.links} key={`${i}${item.name}i`} > <li className="mb-9 cursor-pointer" dangerouslySetInnerHTML={{ __html: item.image }} /></Link>

                    ))
                }

            </ul>
            <div className=" border-[#EBE9E1] border-t-[1px] flex justify-center mb-2">
