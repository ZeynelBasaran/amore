    ...
    return (
        <section className="flex justify-center gap-7 mt-12 relative">
            ...
            {premiumPlans.map((item, i) => (
                <div key={`${i}-${item.price}`} className="w-[307px] h-[539px]  bg-white rounded-2xl">

                    <div className={`rounded-t-2xl w-full h-[120px] p-8 relative ${item.bg}`}>
                        <h3 className="text-2xl font-bold">{item.duration}</h3>
                        <p className="text-lg font-semibold text-basitplan">{item.name}</p>
                        <span dangerouslySetInnerHTML={{ __html: item.svg }} className="absolute right-5 bottom-0" />
                    </div>
